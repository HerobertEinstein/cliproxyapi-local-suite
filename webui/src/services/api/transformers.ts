import type {
  ...
  LogicalModelGroupConfig,
  LogicalModelGroupsConfig,
  LogicalModelGroupReasoningConfig
} from '@/types';
...
const normalizeLogicalModelGroupReasoning = (
  input: unknown
): LogicalModelGroupReasoningConfig | undefined => {
  if (!isRecord(input)) return undefined;
  const mode = input.mode ?? input['mode'];
  const effort = input.effort ?? input['effort'];
  const normalized: LogicalModelGroupReasoningConfig = {};
  if (typeof mode === 'string' && mode.trim()) {
    normalized.mode = mode.trim();
  }
  if (typeof effort === 'string' && effort.trim()) {
    normalized.effort = effort.trim();
  }
  return Object.keys(normalized).length ? normalized : undefined;
};

const normalizeLogicalModelGroup = (input: unknown): LogicalModelGroupConfig | null => {
  ...
  return {
    alias,
    target,
    reasoning: normalizeLogicalModelGroupReasoning(input.reasoning)
  };
};

const normalizeLogicalModelCurrent = (input: unknown): LogicalModelCurrentConfig | null => {
  if (!isRecord(input)) return null;
  const alias = String(input.alias ?? 'current').trim() || 'current';
  const ref = String(input.ref ?? '').trim();
  return { alias, ref };
};

const normalizeLogicalModelGroups = (payload: unknown): LogicalModelGroupsConfig | undefined => {
  const source = isRecord(payload)
    ? ((payload['logical-model-groups'] ?? payload.logicalModelGroups ?? payload) as unknown)
    : payload;
  if (!isRecord(source)) return undefined;
  const current = normalizeLogicalModelCurrent(source.current);
  if (!current) return undefined;
  const staticGroups = Array.isArray(source.static)
    ? source.static
        .map((item) => normalizeLogicalModelGroup(item))
        .filter(Boolean) as LogicalModelGroupConfig[]
    : [];
  return { current, static: staticGroups };
};
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const logicalModelGroups = normalizeLogicalModelGroups(
    raw['logical-model-groups'] ?? raw.logicalModelGroups
  );
  if (logicalModelGroups) {
    config.logicalModelGroups = logicalModelGroups;
  }
