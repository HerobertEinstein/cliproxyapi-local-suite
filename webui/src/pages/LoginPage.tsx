    ...
    const init = async () => {
      try {
        ...
        if (autoLoggedIn) {
          ...
          setApiBase(latestAuth.apiBase || storedBase || detectedBase);
          setManagementKey(latestAuth.managementKey || storedKey || '');
          setRememberPassword(
            latestAuth.rememberPassword ||
              storedRememberPassword ||
              Boolean(latestAuth.managementKey || storedKey)
          );
        }
      } finally {
        if (!autoLoginSuccess) {
