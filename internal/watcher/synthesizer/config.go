...
func (s *ConfigSynthesizer) synthesizeGeminiKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.GeminiKey {
		...
		attrs := map[string]string{
			...
			"api_key":        key,
			"config_locator": fmt.Sprintf("gemini-api-key[%d]", i),
		}
		if entry.Priority != 0 {
...
func (s *ConfigSynthesizer) synthesizeClaudeKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.ClaudeKey {
		...
		attrs := map[string]string{
			...
			"api_key":        key,
			"config_locator": fmt.Sprintf("claude-api-key[%d]", i),
		}
		if ck.Priority != 0 {
...
func (s *ConfigSynthesizer) synthesizeCodexKeys(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.CodexKey {
		...
		attrs := map[string]string{
			...
			"api_key":        key,
			"config_locator": fmt.Sprintf("codex-api-key[%d]", i),
		}
		if ck.Priority != 0 {
			attrs["priority"] = strconv.Itoa(ck.Priority)
		}
	...
	for i := range cfg.OpenAICompatibility {
		...
		for j := range compat.APIKeyEntries {
			...
			attrs := map[string]string{
				...
				"config_locator": fmt.Sprintf("openai-compatibility[%d].api-key-entries[%d]", i, j),
			}
		...
		if createdEntries == 0 {
			...
			attrs := map[string]string{
				...
				"compat_name":    compat.Name,
				"provider_key":   providerName,
				"config_locator": fmt.Sprintf("openai-compatibility[%d]", i),
			}
			if compat.Priority != 0 {
				attrs["priority"] = strconv.Itoa(compat.Priority)
			}
...
func (s *ConfigSynthesizer) synthesizeVertexCompat(ctx *SynthesisContext) []*coreauth.Auth {
	...
	for i := range cfg.VertexCompatAPIKey {
		...
		attrs := map[string]string{
			...
			"base_url":       base,
			"provider_key":   providerName,
			"config_locator": fmt.Sprintf("vertex-api-key[%d]", i),
		}
		if compat.Priority != 0 {
			attrs["priority"] = strconv.Itoa(compat.Priority)
		}
