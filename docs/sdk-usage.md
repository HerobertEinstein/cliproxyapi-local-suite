...
- `openai-compatibility` model discovery follows the effective-model rule: declared `models` stay authoritative, while providers without declared `models` can refresh their discovered `/v1/models` or `/models` cache during startup and config reload.
