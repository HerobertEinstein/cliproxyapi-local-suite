...
import { ampcodeApi, providerHealthApi, providersApi } from '@/services/api';
...
import { runWithProviderHealthRefresh } from './aiProvidersHealthRefresh';
...
export function AiProvidersPage() {
  ...
  const [healthProviders, setHealthProviders] = useState<ProviderHealthProvider[]>([]);
  ...
  const displayState = useMemo(
    ...
    [claudeConfigs, codexConfigs, geminiKeys, healthProviders, openaiProviders, vertexConfigs]
  );
  ...
  const loadConfigs = useCallback(async () => {
    ...
    setError('');
    try {
      const [configResult, vertexResult, ampcodeResult, providerHealthResult] =
        await Promise.allSettled([
        fetchConfig(),
        providersApi.getVertexConfigs(),
        ampcodeApi.getAmpcode(),
        providerHealthApi.getProviders(),
        ]);
      ...
      if (providerHealthResult.status === 'fulfilled') {
        setHealthProviders(providerHealthResult.value?.providers || []);
      }
    } catch (err: unknown) {
  ...
  const setConfigEnabled = async (
    ...
    if (provider === 'gemini') {
      ...
      try {
        await runWithProviderHealthRefresh({
          mutation: () => providersApi.saveGeminiKeys(nextList),
          setHealthProviders,
        });
        showNotification(
    ...
    try {
      if (provider === 'codex') {
        await runWithProviderHealthRefresh({
          mutation: () => providersApi.saveCodexConfigs(nextList),
          setHealthProviders,
        });
      } else if (provider === 'claude') {
        await runWithProviderHealthRefresh({
          mutation: () => providersApi.saveClaudeConfigs(nextList),
          setHealthProviders,
        });
      } else {
        await runWithProviderHealthRefresh({
          mutation: () => providersApi.saveVertexConfigs(nextList),
          setHealthProviders,
        });
      }
      showNotification(
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        try {
          if (type === 'codex') {
            await runWithProviderHealthRefresh({
              mutation: () => providersApi.deleteCodexConfig(entry.apiKey),
              setHealthProviders,
            });
            ...
            await runWithProviderHealthRefresh({
              mutation: () => providersApi.deleteClaudeConfig(entry.apiKey),
              setHealthProviders,
            });
  ...
  const deleteVertex = async (index: number) => {
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        try {
          await runWithProviderHealthRefresh({
            mutation: () => providersApi.deleteVertexConfig(entry.apiKey),
            setHealthProviders,
          });
  ...
  const deleteOpenai = async (index: number) => {
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        try {
          await runWithProviderHealthRefresh({
            mutation: () => providersApi.deleteOpenAIProvider(entry.name),
            setHealthProviders,
          });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        ...
        <div id="provider-gemini">
          <GeminiSection
            configs={displayState.geminiKeys}
            keyStats={keyStats}
        ...
        <div id="provider-codex">
          <CodexSection
            configs={displayState.codexConfigs}
            keyStats={keyStats}
            usageDetails={usageDetails}
            loading={loading}
            disableControls={disableControls}
        ...
        <div id="provider-claude">
          <ClaudeSection
            configs={displayState.claudeConfigs}
            keyStats={keyStats}
            usageDetails={usageDetails}
            loading={loading}
            disableControls={disableControls}
        ...
        <div id="provider-vertex">
          <VertexSection
            configs={displayState.vertexConfigs}
            keyStats={keyStats}
            usageDetails={usageDetails}
            loading={loading}
            disableControls={disableControls}
        ...
        <div id="provider-openai">
          <OpenAISection
            configs={displayState.openaiProviders}
            keyStats={keyStats}
            usageDetails={usageDetails}
            loading={loading}
            disableControls={disableControls}
