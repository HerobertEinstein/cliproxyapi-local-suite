...
func (a *IFlowAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	authURL, redirectURI, err := authSvc.AuthorizationURL(state, callbackPort)
	if err != nil {
		return nil, fmt.Errorf("iflow auth: oauth credentials unavailable: %w", err)
	}
...
waitForCallback:
	...
	metadata := oauthcreds.Apply(map[string]any{
		...
		ClientSecret: tokenStorage.ClientSecret,
	})
