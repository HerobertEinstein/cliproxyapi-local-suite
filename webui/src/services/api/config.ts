...
export const configApi = {
  ...
  async getLogicalModelGroups(): Promise<LogicalModelGroupsConfig> {
    const raw = await apiClient.get<Record<string, unknown>>('/logical-model-groups');
    const payload =
      (raw?.['logical-model-groups'] as Record<string, unknown> | undefined) ??
      (raw as Record<string, unknown>);
    const normalized = normalizeConfigResponse({ 'logical-model-groups': payload });
    return (
      normalized.logicalModelGroups ?? {
  ...
  updateLogicalModelGroupCurrent: (ref: string) =>
    apiClient.put('/logical-model-groups/current', { ref }),

  createLogicalModelGroupStatic: (group: LogicalModelGroupConfig) =>
    apiClient.post('/logical-model-groups/static', group),

  deleteLogicalModelGroupStatic: (alias: string) =>
    apiClient.delete(`/logical-model-groups/static/${encodeURIComponent(alias)}`),
};
