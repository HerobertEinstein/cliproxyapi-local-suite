...
func (ia *IFlowAuth) ExchangeCodeForTokens(ctx context.Context, code, redirectURI string) (*IFlowTokenData, error) {
	...
	req, err := ia.newTokenRequest(ctx, form, clientID, clientSecret)
	if err != nil {
		return nil, err
	}
...
func (ia *IFlowAuth) newTokenRequest(ctx context.Context, form url.Values, clientID, clientSecret string) (*http.Request, error) {
