...
func (h *Handler) refreshGeminiOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	creds, errCreds := oauthcreds.Gemini.Resolve(metadata)
	if errCreds != nil {
		return "", errCreds
	}
	base = oauthcreds.Apply(base, creds)
	...
	fields = oauthcreds.Apply(fields, creds)
	if updater != nil {
		updater(fields)
	}
...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	creds, errCreds := oauthcreds.Antigravity.Resolve(metadata)
	if errCreds != nil {
		return "", errCreds
	}
	...
	auth.Metadata = oauthcreds.Apply(auth.Metadata, creds)
	now := time.Now()
