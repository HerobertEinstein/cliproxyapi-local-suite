...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	if errCreds != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"status": "error", "error": errCreds.Error()})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	if strings.TrimSpace(authURL) == "" {
		_, _, errCreds := antigravity.OAuthClientCredentials()
		if errCreds == nil {
			errCreds = fmt.Errorf("failed to build antigravity auth url")
		}
...
func (h *Handler) RequestIFlowToken(c *gin.Context) {
	...
	if errAuthURL != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"status": "error", "error": errAuthURL.Error()})
		return
	}
