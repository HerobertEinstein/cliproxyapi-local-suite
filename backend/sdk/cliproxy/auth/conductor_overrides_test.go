...
type streamBootstrapFallbackExecutor struct {
	id string

	mu              sync.Mutex
	streamCalls     []string
	bootstrapStalls map[string]time.Duration
}

func (e *streamBootstrapFallbackExecutor) Identifier() string {
	return e.id
}

func (e *streamBootstrapFallbackExecutor) Execute(context.Context, *Auth, cliproxyexecutor.Request, cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	return cliproxyexecutor.Response{}, &Error{HTTPStatus: 500, Message: "not implemented"}
}

func (e *streamBootstrapFallbackExecutor) ExecuteStream(ctx context.Context, auth *Auth, _ cliproxyexecutor.Request, _ cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	e.mu.Lock()
	e.streamCalls = append(e.streamCalls, auth.ID)
	stall := e.bootstrapStalls[auth.ID]
	e.mu.Unlock()
...
func (e *streamBootstrapFallbackExecutor) Refresh(_ context.Context, auth *Auth) (*Auth, error) {
	return auth, nil
}

func (e *streamBootstrapFallbackExecutor) CountTokens(context.Context, *Auth, cliproxyexecutor.Request, cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	return cliproxyexecutor.Response{}, &Error{HTTPStatus: 500, Message: "not implemented"}
}

func (e *streamBootstrapFallbackExecutor) HttpRequest(context.Context, *Auth, *http.Request) (*http.Response, error) {
	return nil, nil
}

func (e *streamBootstrapFallbackExecutor) StreamCalls() []string {
	e.mu.Lock()
	defer e.mu.Unlock()
	out := make([]string, len(e.streamCalls))
	copy(out, e.streamCalls)
...
func TestManager_ModelSupportUpstreamNoClientsAvailable_FallsBackAndSuspendsAuth(t *testing.T) {
	m := NewManager(nil, nil, nil)
	executor := &authFallbackExecutor{
		id: "claude",
		executeErrors: map[string]error{
	...
	if remaining := time.Until(state.NextRetryAfter); remaining < 11*time.Hour {
		t.Fatalf("expected model-support cooldown close to 12h, got %v", remaining)
	}
}
...
func TestManagerExecuteStream_BootstrapTimeoutFallsBackAndSuspendsAuth(t *testing.T) {
	prevTimeout := streamBootstrapTimeout
	streamBootstrapTimeout = 20 * time.Millisecond
	t.Cleanup(func() {
		streamBootstrapTimeout = prevTimeout
	})

	m := NewManager(nil, nil, nil)
	executor := &streamBootstrapFallbackExecutor{
		id: "claude",
		bootstrapStalls: map[string]time.Duration{
			"aa-slow-auth": 500 * time.Millisecond,
		},
	}
	m.RegisterExecutor(executor)
	...
	streamResult, errExecute := m.ExecuteStream(ctx, []string{"claude"}, cliproxyexecutor.Request{Model: model}, cliproxyexecutor.Options{})
	if errExecute != nil {
	...
	if !state.Unavailable {
		t.Fatalf("expected bootstrap-timeout auth model state to be unavailable")
	}
	if state.NextRetryAfter.IsZero() {
		t.Fatalf("expected bootstrap-timeout auth model state cooldown to be set")
	}
}
