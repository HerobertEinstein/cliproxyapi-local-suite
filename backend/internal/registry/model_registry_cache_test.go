...
func TestGetAvailableModelsRestoresModelAfterSuspensionExpires(t *testing.T) {
	r := newTestModelRegistry()
	r.RegisterClient("client-1", "OpenAI", []*ModelInfo{{ID: "m1", OwnedBy: "team-a", DisplayName: "Model One"}})

	r.SuspendClientModelUntil("client-1", "m1", "manual", time.Now().Add(20*time.Millisecond))
	if models := r.GetAvailableModels("openai"); len(models) != 0 {
		t.Fatalf("expected model to be hidden during suspension, got %d", len(models))
	}
	...
	models := r.GetAvailableModels("openai")
	if len(models) != 1 {
		t.Fatalf("expected model to return after suspension expiry, got %d", len(models))
	}
	if count := r.GetModelCount("m1"); count != 1 {
		t.Fatalf("expected model count 1 after suspension expiry, got %d", count)
	}
}
