...
func (AntigravityAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	authURL, err := authSvc.BuildAuthURL(state, redirectURI)
	if err != nil {
		return nil, fmt.Errorf("antigravity: oauth credentials unavailable: %w", err)
	}
...
waitForCallback:
	...
	creds, err := oauthcreds.Antigravity.Resolve(nil)
	if err != nil {
		return nil, fmt.Errorf("antigravity: oauth credentials unavailable: %w", err)
	}
