...
func TestRescanConfigProviders_StopsAfterEmptyJSONModelsResponse(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/models":
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"data":[],"object":"list","success":true}`))
		case "/models":
			fallbackHits.Add(1)
	...
	statuses, err := RescanConfigProviders(context.Background(), cfg, nil)
	if err != nil {
		t.Fatalf("rescan providers: %v", err)
	}
	if fallbackHits.Load() != 0 {
		t.Fatalf("did not expect fallback /models probe after empty JSON response, got %d hits", fallbackHits.Load())
	}
	if len(statuses) != 1 {
		t.Fatalf("expected 1 status, got %d", len(statuses))
	}
	if statuses[0].DiscoveryState != "stale" {
		t.Fatalf("expected stale discovery state, got %q", statuses[0].DiscoveryState)
	}
	if statuses[0].EffectiveSource != "stale-cache" {
		t.Fatalf("expected stale-cache effective source, got %q", statuses[0].EffectiveSource)
	}
	if len(statuses[0].EffectiveModels) != 1 || statuses[0].EffectiveModels[0] != "gpt-5.4" {
		t.Fatalf("expected stale cached effective models, got %#v", statuses[0].EffectiveModels)
	}
	if !strings.Contains(statuses[0].LastError, "/v1/models: models array is empty") {
		t.Fatalf("expected empty-array error from preferred endpoint, got %q", statuses[0].LastError)
	}
}

func TestRescanConfigProviders_StopsAfterAuthFailureOnPreferredModelsEndpoint(t *testing.T) {
	var fallbackHits atomic.Int32
	...
	if fallbackHits.Load() != 0 {
		t.Fatalf("did not expect fallback /models probe after auth failure, got %d hits", fallbackHits.Load())
	}
	if len(statuses) != 1 {
		t.Fatalf("expected 1 status, got %d", len(statuses))
	}
	if statuses[0].DiscoveryState != "error" {
		t.Fatalf("expected error discovery state, got %q", statuses[0].DiscoveryState)
	}
	if !strings.Contains(statuses[0].LastError, "/v1/models: status 401") {
		t.Fatalf("expected auth failure from preferred endpoint, got %q", statuses[0].LastError)
	}
}

func TestRescanConfigProviders_PreservesPrimaryEndpointErrorWhenFallbackAlsoFails(t *testing.T) {
	var fallbackHits atomic.Int32
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/models":
			...
			_, _ = w.Write([]byte(`dns lookup failed`))
		case "/models":
			fallbackHits.Add(1)
			w.Header().Set("Content-Type", "text/html; charset=utf-8")
			w.WriteHeader(http.StatusBadGateway)
			_, _ = w.Write([]byte(`<!doctype html><html><body>frontend fallback</body></html>`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if fallbackHits.Load() != 1 {
		t.Fatalf("expected fallback /models probe once, got %d", fallbackHits.Load())
	}
	if len(statuses) != 1 {
		t.Fatalf("expected 1 status, got %d", len(statuses))
	}
	...
	if !strings.Contains(statuses[0].LastError, "/v1/models: status 502") {
		t.Fatalf("expected primary endpoint error to be retained, got %q", statuses[0].LastError)
	}
	if strings.Contains(statuses[0].LastError, server.URL+"/models: status 502") {
		t.Fatalf("did not expect fallback endpoint error to replace primary error, got %q", statuses[0].LastError)
	}
}
