...
type Config struct {
	...
	// LogicalModelGroups defines client-visible logical model groups projected onto runtime alias chains.
	LogicalModelGroups LogicalModelGroups `yaml:"logical-model-groups,omitempty" json:"logical-model-groups,omitempty"`

	// ConfigFilePath stores the runtime source path of the loaded config file.
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Normalize logical model groups.
	...
	cfg.ConfigFilePath = strings.TrimSpace(configFile)
...
func SaveConfigPreserveComments(configFile string, cfg *Config) error {
	...
	removeLegacyLogicalModelGroupCurrentKeys(original.Content[0])
...
func removeLegacyLogicalModelGroupCurrentKeys(root *yaml.Node) {
	if root == nil || root.Kind != yaml.MappingNode {
		return
	}
	logicalIdx := findMapKeyIndex(root, "logical-model-groups")
	if logicalIdx < 0 || logicalIdx+1 >= len(root.Content) {
		return
	}
	logicalNode := root.Content[logicalIdx+1]
	if logicalNode == nil || logicalNode.Kind != yaml.MappingNode {
		return
	}
	currentIdx := findMapKeyIndex(logicalNode, "current")
	if currentIdx < 0 || currentIdx+1 >= len(logicalNode.Content) {
		return
	}
	currentNode := logicalNode.Content[currentIdx+1]
	if currentNode == nil || currentNode.Kind != yaml.MappingNode {
		return
	}
	removeMapKey(currentNode, "target")
	removeMapKey(currentNode, "reasoning")
}
