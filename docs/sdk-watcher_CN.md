...
- 配置热重载还可能在 reload 回调中触发服务侧的派生刷新逻辑。例如，对未声明 `models` 的 `openai-compatibility` provider，会在这里重新扫描，使其自动发现得到的模型缓存与最新配置保持一致。
...
对 `openai-compatibility` 而言，设计 reload 路径时要区分三件事：

- 已手写声明的 `openai-compatibility.models` 始终是权威模型集。
- 未声明 `models` 的 provider，可以在启动和配置热重载时刷新自动发现得到的 `/v1/models` 或 `/models` 缓存。
- 热重载应只重新扫描“新增或已变更且仍未声明 `models`”的 provider，而不是每次都全量重扫所有 `openai-compatibility` provider。
