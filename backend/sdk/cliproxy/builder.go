...
func (b *Builder) Build() (*Service, error) {
	...
	b.cfg.ConfigFilePath = b.configPath
	...
	if coreManager == nil {
		...
		runtimeStateHook := &authRuntimeStateHook{configPath: b.configPath}
		coreManager = coreauth.NewManager(tokenStore, selector, runtimeStateHook)
		runtimeStateHook.manager = coreManager
	}
	// Attach a default RoundTripper provider so providers can opt-in per-auth transports.
	coreManager.SetRoundTripperProvider(newDefaultRoundTripperProvider())
	coreManager.SetConfig(b.cfg)
	coreManager.SetOAuthModelAlias(b.cfg.ProjectedOAuthModelAliasTable())
