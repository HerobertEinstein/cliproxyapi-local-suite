...
func (ia *IFlowAuth) AuthorizationURL(state string, port int) (authURL, redirectURI string, err error) {
	redirectURI = fmt.Sprintf("http://localhost:%d/oauth2callback", port)
	creds, err := oauthcreds.IFlow.Resolve(nil)
	if err != nil {
...
func (ia *IFlowAuth) ExchangeCodeForTokens(ctx context.Context, code, redirectURI string) (*IFlowTokenData, error) {
	...
	req, err := ia.newTokenRequest(ctx, form, creds)
	if err != nil {
		return nil, err
	}

	return ia.doTokenRequest(ctx, req, creds)
}

// RefreshTokens exchanges a refresh token for a new access token.
func (ia *IFlowAuth) RefreshTokens(ctx context.Context, refreshToken string, metadata map[string]any) (*IFlowTokenData, error) {
	creds, err := oauthcreds.IFlow.Resolve(metadata)
	if err != nil {
		return nil, err
	}
	form := url.Values{}
...
func (ia *IFlowAuth) newTokenRequest(ctx context.Context, form url.Values, creds oauthcreds.Credentials) (*http.Request, error) {
...
func (ia *IFlowAuth) doTokenRequest(ctx context.Context, req *http.Request, creds oauthcreds.Credentials) (*IFlowTokenData, error) {
