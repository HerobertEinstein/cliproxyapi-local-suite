...
- Configuration reloads may also trigger service-level derived refresh work in the reload callback. For example, `openai-compatibility` providers without declared `models` are rescanned there so their discovered model cache stays aligned with the latest config.
...
For `openai-compatibility`, keep this distinction in mind when designing your reload path:

- Declared `openai-compatibility.models` remain authoritative.
- Providers without declared `models` can refresh their discovered `/v1/models` or `/models` cache during startup and config reload.
- Reload should only rescan added or changed undeclared providers, rather than rescanning every `openai-compatibility` provider on each change.
