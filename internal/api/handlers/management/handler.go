...
func NewHandler(cfg *config.Config, configFilePath string, manager *coreauth.Manager) *Handler {
	...
	if cfg != nil && strings.TrimSpace(cfg.ConfigFilePath) == "" {
		cfg.ConfigFilePath = strings.TrimSpace(configFilePath)
	}
...
func (h *Handler) SetConfig(cfg *config.Config) {
	if cfg != nil && strings.TrimSpace(cfg.ConfigFilePath) == "" {
		cfg.ConfigFilePath = strings.TrimSpace(h.configFilePath)
	}
	h.cfg = cfg
}
...
// SetRuntimeRefreshHook registers a hook to refresh runtime state after management updates.
func (h *Handler) SetRuntimeRefreshHook(hook func(context.Context)) {
	h.runtimeRefreshHook = hook
}
...
func (h *Handler) persistWithConfigProviderRefresh(c *gin.Context, keys []string) bool {
...
func (h *Handler) persistWithConfigProviderRefreshPlan(c *gin.Context, plan configProviderRefreshPlan) bool {
	...
	if len(plan.PruneKeys) > 0 {
		configPath := ""
		if h.cfg != nil {
			configPath = strings.TrimSpace(h.cfg.ConfigFilePath)
		}
		if err := modeldiscovery.PruneConfigProviderCache(configPath, plan.PruneKeys); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to prune provider discovery cache: %v", err)})
			return false
		}
	}

	if len(plan.RescanKeys) > 0 {
		timeoutCtx, cancel := context.WithTimeout(ctx, 30*time.Second)
		defer cancel()

		if _, err := modeldiscovery.RescanConfigProviders(timeoutCtx, h.cfg, plan.RescanKeys); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("failed to refresh provider discovery: %v", err)})
			return false
		}
	}
