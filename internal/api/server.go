...
type serverOptionConfig struct {
	...
	runtimeRefreshHook   func(context.Context)
}
...
// WithRuntimeRefreshHook registers a hook invoked after management-side runtime catalog updates.
func WithRuntimeRefreshHook(hook func(context.Context)) ServerOption {
	return func(cfg *serverOptionConfig) {
		cfg.runtimeRefreshHook = hook
	}
}
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if optionState.runtimeRefreshHook != nil {
		s.mgmt.SetRuntimeRefreshHook(optionState.runtimeRefreshHook)
	}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/logical-model-groups", s.mgmt.GetLogicalModelGroups)
		mgmt.PUT("/logical-model-groups/current", s.mgmt.PutLogicalModelGroupCurrent)
		mgmt.PATCH("/logical-model-groups/current", s.mgmt.PutLogicalModelGroupCurrent)
		mgmt.POST("/logical-model-groups/static", s.mgmt.PostLogicalModelGroupStatic)
		mgmt.DELETE("/logical-model-groups/static/:alias", s.mgmt.DeleteLogicalModelGroupStatic)
		mgmt.GET("/provider-model-discovery", s.mgmt.GetProviderModelDiscovery)
		mgmt.POST("/provider-model-discovery/rescan", s.mgmt.PostProviderModelDiscoveryRescan)
		mgmt.GET("/provider-health", s.mgmt.GetProviderHealth)
		mgmt.POST("/provider-health/reset", s.mgmt.PostProviderHealthReset)
		mgmt.GET("/openai-compatibility/discovery", s.mgmt.GetOpenAICompatDiscovery)
		mgmt.POST("/openai-compatibility/discovery/rescan", s.mgmt.PostOpenAICompatDiscoveryRescan)
		...
		mgmt.GET("/codex/import-health", s.mgmt.GetCodexImportHealth)
