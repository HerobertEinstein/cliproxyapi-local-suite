...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v6/internal/auth/oauthcreds"
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	auth.EnsureIndex()
	configSource := strings.TrimSpace(authAttribute(auth, "source"))
	configBacked := strings.HasPrefix(strings.ToLower(configSource), "config:")
	runtimeOnly := isRuntimeOnlyAuth(auth)
	if runtimeOnly && !configBacked && (auth.Disabled || auth.Status == coreauth.StatusDisabled) {
		return nil
	}
	path := strings.TrimSpace(authAttribute(auth, "path"))
	if path == "" && !runtimeOnly && !configBacked {
		return nil
	}
	...
	if configBacked {
		entry["source"] = configSource
	}
	if auth.ID != "" {
		entry["model_count"] = len(registry.GetGlobalRegistry().GetModelsForClient(auth.ID))
	}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	creds, errCreds := oauthcreds.Gemini.Resolve(nil)
	if errCreds != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "oauth credentials unavailable"})
		return
	}

	// OAuth2 configuration using resolved credentials
	conf := &oauth2.Config{
	...
	go func() {
		...
		ifToken = oauthcreds.Apply(ifToken, creds)
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	creds, errCreds := oauthcreds.Antigravity.Resolve(nil)
	if errCreds != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "oauth credentials unavailable"})
		return
	}
	...
	if errAuthURL != nil {
		log.Errorf("Failed to build auth url: %v", errAuthURL)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "oauth credentials unavailable"})
		return
	}
	...
	go func() {
		...
		now := time.Now()
		metadata := oauthcreds.Apply(map[string]any{
			"type":          "antigravity",
...
func (h *Handler) RequestIFlowToken(c *gin.Context) {
	...
	creds, errCreds := oauthcreds.IFlow.Resolve(nil)
	if errCreds != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"status": "error", "error": "oauth credentials unavailable"})
		return
	}
	authURL, redirectURI, errAuthURL := authSvc.AuthorizationURL(state, iflowauth.CallbackPort)
	if errAuthURL != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"status": "error", "error": "oauth credentials unavailable"})
		return
	}
