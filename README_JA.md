...
このリポジトリは個人用のローカルスイート向けフォークであり、公式の upstream リポジトリではありません。

upstream の CLIProxyAPI と比べると、このフォークは Windows ローカル運用、スクリプトによる保守、そしてより安全なセルフホスト型ルーティングに重点を置いています。

- 予約済みの動的ポインタ `current` と、編集可能な静的グループから成る論理モデルグループ
- `models` を宣言していない `openai-compatibility` provider に対する自動モデル検出
- 設定リロード時に、追加または変更された未宣言 provider のみを対象にした増分再スキャン
- Web UI / スクリプトから `current` の切り替えや静的論理グループ管理を行うための Management API
- 同一の実 Edge セッションへ安定して自動接続するための Windows Web UI 起動チェーン / Edge CDP ガイダンス
- Gemini / Antigravity / iFlow の OAuth client credentials をハードコードではなく、auth metadata または環境変数から解決する方式へ変更

元の挙動や公式のリリース運用を使いたい場合は、公式 upstream リポジトリを利用してください。
...
## Windows GUI 起動チェーンと Edge CDP

Windows でショートカット、トレイエントリ、補助スクリプト経由で内蔵 Web UI を開きつつ、ブラウザ自動化ツールを**同じ実際の Edge セッション**に接続したい場合は、起動チェーン全体を統一する必要があります。

- Microsoft Edge を `--remote-debugging-port=9333` 付きで起動する
- `--app=...` のような二次ラッパーでそのフラグを落とさない
- 独自ランチャーを使う場合も、app モードと通常ウィンドウモードの両方で同じリモートデバッグポートを渡す

重要なのは、Playwright / CDP が接続できるかどうかは、想定していたショートカットではなく、**実際に起動された Edge プロセスのコマンドライン**で決まるという点です。補助スクリプトが `msedge.exe --app=...` を直接起動し、リモートデバッグポートを付けていない場合、Web UI 自体は開いても、自動化ツールはその既存 Edge ウィンドウに接続できません。
