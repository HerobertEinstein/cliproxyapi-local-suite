...
var streamBootstrapTimeout = 10 * time.Second
...
func readStreamBootstrap(ctx context.Context, ch <-chan cliproxyexecutor.StreamChunk) ([]cliproxyexecutor.StreamChunk, bool, error) {
	...
	if streamBootstrapTimeout > 0 {
		timeoutTimer = time.NewTimer(streamBootstrapTimeout)
		...
		if ctx != nil {
			...
			case <-timeoutCh:
				return nil, false, &Error{
					...
					Message:    "upstream stream bootstrap timed out before first payload",
					Retryable:  true,
	...
	for _, auth := range m.auths {
		...
		case "gemini":
			if entry := resolveGeminiAPIKeyConfig(cfg, auth); entry != nil {
				...
				compileAPIKeyLogicalModelGroups(byAlias, cfg, asModelAliasEntries(entry.Models))
				compileAPIKeyLogicalModelGroups(stateAliases, cfg, asModelAliasEntries(entry.Models))
			}
		case "claude":
		...
		default:
			...
			if compatName != "" || strings.EqualFold(strings.TrimSpace(auth.Provider), "openai-compatibility") {
				if entry := resolveOpenAICompatConfig(cfg, providerKey, compatName, auth.Provider); entry != nil {
					models := modeldiscovery.EffectiveOpenAICompatModels(cfg, entry)
					compileAPIKeyModelAliasForModels(byAlias, models)
					compileAPIKeyLogicalModelGroups(byAlias, cfg, asModelAliasEntries(models))
					compileAPIKeyLogicalModelGroups(stateAliases, cfg, asModelAliasEntries(models))
				}
			}
		}
		...
		auth.APIKeyModelAliases = cloneStringMap(stateAliases)
	}

	m.apiKeyModelAlias.Store(out)
}

func compileAPIKeyLogicalModelGroups(byAlias map[string]string, cfg *internalconfig.Config, models []modelAliasEntry) {
	if cfg == nil {
		return
	}
	noCapabilityList := len(models) == 0
	for _, group := range cfg.LogicalModelGroupEntries() {
		aliasKey := strings.ToLower(strings.TrimSpace(group.Alias))
		if aliasKey == "" {
			continue
		}
		if _, exists := byAlias[aliasKey]; exists {
			continue
		}
		resolved := ""
		if noCapabilityList {
			resolved = group.ResolvedTarget()
		} else {
			resolved = resolveModelAliasFromConfigModels(group.ResolvedTarget(), models)
		}
		if resolved == "" {
			continue
		}
		byAlias[aliasKey] = resolved
	}
}
...
func authConfigLocator(auth *Auth) string {
...
func resolveConfigEntryByLocator[T any](entries []T, auth *Auth, prefix string) *T {
	locator := authConfigLocator(auth)
	if locator == "" || prefix == "" {
		return nil
	}
...
func resolveGeminiAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.GeminiKey {
	...
	if entry := resolveConfigEntryByLocator(cfg.GeminiKey, auth, "gemini-api-key"); entry != nil {
		return entry
	}
...
func resolveClaudeAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.ClaudeKey {
	...
	if entry := resolveConfigEntryByLocator(cfg.ClaudeKey, auth, "claude-api-key"); entry != nil {
		return entry
	}
...
func resolveCodexAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.CodexKey {
	...
	if entry := resolveConfigEntryByLocator(cfg.CodexKey, auth, "codex-api-key"); entry != nil {
		return entry
	}
...
func resolveVertexAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.VertexCompatKey {
	...
	if entry := resolveConfigEntryByLocator(cfg.VertexCompatAPIKey, auth, "vertex-api-key"); entry != nil {
		return entry
	}
...
func resolveUpstreamModelForGeminiAPIKey(cfg *internalconfig.Config, auth *Auth, requestedModel string) string {
	...
	if entry == nil {
		return cfg.ResolveLogicalModelGroup(requestedModel)
	}
	resolved := resolveModelAliasFromConfigModels(requestedModel, asModelAliasEntries(entry.Models))
	if resolved != "" {
		return resolved
	}
	return resolveLogicalModelGroupForEntries(cfg, requestedModel, asModelAliasEntries(entry.Models))
}
...
func resolveUpstreamModelForOpenAICompatAPIKey(cfg *internalconfig.Config, auth *Auth, requestedModel string) string {
	...
	models := modeldiscovery.EffectiveOpenAICompatModels(cfg, entry)
	resolved := resolveModelAliasFromConfigModels(requestedModel, asModelAliasEntries(models))
	if resolved != "" {
		return resolved
	}
	return resolveLogicalModelGroupForEntries(cfg, requestedModel, asModelAliasEntries(models))
}

func resolveLogicalModelGroupForEntries(cfg *internalconfig.Config, requestedModel string, entries []modelAliasEntry) string {
	if cfg == nil {
		return ""
	}
	requestedModel = strings.TrimSpace(requestedModel)
	...
	for _, group := range cfg.LogicalModelGroupEntries() {
		if !strings.EqualFold(strings.TrimSpace(group.Alias), requestedModel) {
			continue
		}
		return resolveModelAliasFromConfigModels(group.ResolvedTarget(), entries)
	}
	return ""
}
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	trackedModel := strings.TrimSpace(result.Model)
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		now := time.Now()
		if canonical := authCanonicalStateModel(auth, trackedModel); canonical != "" {
			trackedModel = canonical
		}

		if result.Success {
			if trackedModel != "" {
				state := ensureModelState(auth, trackedModel)
	...
	if clearModelQuota && trackedModel != "" {
		registry.GetGlobalRegistry().ClearModelQuotaExceeded(result.AuthID, trackedModel)
	}
	if setModelQuota && trackedModel != "" {
		registry.GetGlobalRegistry().SetModelQuotaExceeded(result.AuthID, trackedModel)
	}
	if shouldResumeModel {
		registry.GetGlobalRegistry().ResumeClientModel(result.AuthID, trackedModel)
	} else if shouldSuspendModel {
		registry.GetGlobalRegistry().SuspendClientModelUntil(result.AuthID, trackedModel, suspendReason, suspendUntil)
	}
