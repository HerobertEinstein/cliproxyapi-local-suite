...
func TestConfigSynthesizer_CodexKeys_AssignsStableConfigLocator(t *testing.T) {
	synth := NewConfigSynthesizer()
	ctx := &SynthesisContext{
		Config: &config.Config{
			CodexKey: []config.CodexKey{
	...
	if got := auths[0].Attributes["config_locator"]; got != "codex-api-key[0]" {
		t.Fatalf("expected first locator %q, got %q", "codex-api-key[0]", got)
	}
	if got := auths[1].Attributes["config_locator"]; got != "codex-api-key[1]" {
		t.Fatalf("expected second locator %q, got %q", "codex-api-key[1]", got)
	}
	if auths[0].Attributes["source"] == auths[1].Attributes["source"] {
		t.Fatalf("expected duplicate config entries to keep unique source tokens, got %q", auths[0].Attributes["source"])
	}
}
