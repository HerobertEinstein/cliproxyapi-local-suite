...
type ModelRegistration struct {
	...
	SuspendedClients map[string]ClientSuspension
}

// ClientSuspension stores temporary model suspension metadata for one client.
type ClientSuspension struct {
	Reason string
	Until  time.Time
}
...
func (r *ModelRegistry) addModelRegistration(modelID, provider string, model *ModelInfo, now time.Time) {
	...
	if existing, exists := r.models[modelID]; exists {
		...
		if existing.SuspendedClients == nil {
			existing.SuspendedClients = make(map[string]ClientSuspension)
		}
	...
	registration := &ModelRegistration{
		...
		SuspendedClients:     make(map[string]ClientSuspension),
	}
	if provider != "" {
...
func (r *ModelRegistry) SuspendClientModel(clientID, modelID, reason string) {
	r.SuspendClientModelUntil(clientID, modelID, reason, time.Time{})
}

// SuspendClientModelUntil marks a client's model as temporarily unavailable until a specific time.
// A zero until timestamp keeps the suspension until explicitly resumed.
func (r *ModelRegistry) SuspendClientModelUntil(clientID, modelID, reason string, until time.Time) {
	if clientID == "" || modelID == "" {
		return
	}
	r.mutex.Lock()
	...
	if registration.SuspendedClients == nil {
		registration.SuspendedClients = make(map[string]ClientSuspension)
	}
	next := ClientSuspension{Reason: reason, Until: until}
	if current, already := registration.SuspendedClients[clientID]; already && current == next {
		return
	}
	registration.SuspendedClients[clientID] = next
	...
	if reason != "" {
		if until.IsZero() {
			...
			log.Debugf("Suspended client %s for model %s until %s: %s", clientID, modelID, until.Format(time.RFC3339), reason)
		}
	} else {
...
func (r *ModelRegistry) buildAvailableModelsLocked(handlerType string, now time.Time) ([]map[string]any, time.Time) {
	...
	for _, registration := range r.models {
		...
		otherSuspended := 0
		if registration.SuspendedClients != nil {
			for clientID, suspension := range registration.SuspendedClients {
				if !suspension.Until.IsZero() {
					if !suspension.Until.After(now) {
						delete(registration.SuspendedClients, clientID)
						continue
					}
					if expiresAt.IsZero() || suspension.Until.Before(expiresAt) {
						expiresAt = suspension.Until
					}
				}
				if strings.EqualFold(suspension.Reason, "quota") {
					cooldownSuspended++
					continue
				}
				otherSuspended++
		...
		if ok && registration != nil {
			...
			if registration.SuspendedClients != nil {
				for clientID, suspension := range registration.SuspendedClients {
					...
					if !suspension.Until.IsZero() && !suspension.Until.After(now) {
						continue
					}
					if strings.EqualFold(suspension.Reason, "quota") {
						cooldownSuspended++
...
func (r *ModelRegistry) GetModelCount(modelID string) int {
	...
	if registration, exists := r.models[modelID]; exists {
		...
		suspendedClients := 0
		if registration.SuspendedClients != nil {
			for _, suspension := range registration.SuspendedClients {
				if !suspension.Until.IsZero() && !suspension.Until.After(now) {
					continue
				}
				suspendedClients++
