...
func authCanonicalStateModel(auth *Auth, model string) string {
	model = strings.TrimSpace(model)
	if model == "" {
		return ""
	}
	...
	if auth != nil && len(auth.APIKeyModelAliases) > 0 {
		...
		if mapped := strings.TrimSpace(auth.APIKeyModelAliases[key]); mapped != "" {
			resolved = preserveRequestedModelSuffix(model, mapped)
		}
	}

	if canonical := canonicalModelKey(resolved); canonical != "" {
		return canonical
	}
	return resolved
}

func authModelStateCandidates(auth *Auth, model string) []string {
	...
	if auth != nil && len(auth.APIKeyModelAliases) > 0 {
		...
		if resolved := strings.TrimSpace(auth.APIKeyModelAliases[key]); resolved != "" {
			resolved = preserveRequestedModelSuffix(model, resolved)
			add(resolved)
			add(canonicalModelKey(resolved))
		}
	}
...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	if model != "" {
		if len(auth.ModelStates) > 0 {
			...
			for _, candidate := range authModelStateCandidates(auth, model) {
				state, ok = auth.ModelStates[candidate]
				if ok && state != nil {
					break
				}
			}
