...
func (s *Service) registerResolvedModelsForAuth(a *coreauth.Auth, providerKey string, models []*ModelInfo) {
	...
	s.syncRegisteredModelStateSuspensions(a, models)
}

func (s *Service) syncRegisteredModelStateSuspensions(a *coreauth.Auth, models []*ModelInfo) {
	if a == nil || a.ID == "" || len(models) == 0 || len(a.ModelStates) == 0 {
		return
	}
	...
	for _, model := range models {
		...
		state := s.lookupRegisteredModelState(a, model.ID)
		...
		if reason == "" && state.LastError != nil {
			reason = strings.TrimSpace(state.LastError.Message)
		}
		modelRegistry.SuspendClientModelUntil(a.ID, model.ID, reason, state.NextRetryAfter)
	}
}

func (s *Service) lookupRegisteredModelState(a *coreauth.Auth, registeredModelID string) *coreauth.ModelState {
	if a == nil || len(a.ModelStates) == 0 {
		return nil
	}

	for _, candidate := range s.registeredModelStateCandidates(a, registeredModelID) {
		if state, ok := a.ModelStates[candidate]; ok && state != nil {
			return state
		}
	}
	return nil
...
func (s *Service) registeredModelStateCandidates(a *coreauth.Auth, registeredModelID string) []string {
	...
	add(registeredModelID)

	baseModelID := strings.TrimSpace(registeredModelID)
	if a != nil {
		prefix := strings.TrimSpace(a.Prefix)
		if prefix != "" {
			prefixed := prefix + "/"
			if strings.HasPrefix(baseModelID, prefixed) {
				baseModelID = strings.TrimSpace(strings.TrimPrefix(baseModelID, prefixed))
			}
		}
	}
	add(baseModelID)
	...
	if s != nil && s.cfg != nil {
		if resolved := strings.TrimSpace(s.cfg.ResolveLogicalModelGroup(baseModelID)); resolved != "" {
			add(resolved)
		}
		if !strings.EqualFold(baseModelID, registeredModelID) {
			if resolved := strings.TrimSpace(s.cfg.ResolveLogicalModelGroup(registeredModelID)); resolved != "" {
				add(resolved)
			}
		}
	}
	...
	for _, candidate := range snapshot {
		if parsed := thinking.ParseSuffix(candidate); strings.TrimSpace(parsed.ModelName) != "" && !strings.EqualFold(parsed.ModelName, candidate) {
...
func (s *Service) Run(ctx context.Context) error {
	...
	options := append([]api.ServerOption(nil), s.serverOptions...)
	options = append(options, api.WithRuntimeRefreshHook(func(ctx context.Context) {
		refreshed := s.refreshAllModelRegistrations()
		if refreshed > 0 {
			log.Infof("re-registered models for %d auth(s) due to management discovery refresh", refreshed)
		}
	}))
	s.server = api.NewServer(s.cfg, s.coreManager, s.accessManager, s.configPath, options...)
	...
	s.refreshOpenAICompatDiscovery(context.Background(), nil, s.cfg)
	...
	reloadCallback := func(newCfg *config.Config) {
		...
		if s.coreManager != nil {
			...
			s.coreManager.SetOAuthModelAlias(newCfg.ProjectedOAuthModelAliasTable())
		}
		s.rebindExecutors()
		s.refreshOpenAICompatDiscovery(context.Background(), previousCfg, newCfg)
		if s.coreManager != nil {
			refreshed := s.refreshAllModelRegistrations()
			if refreshed > 0 {
				log.Infof("re-registered models for %d auth(s) due to config reload", refreshed)
			}
		}
	}
...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	if providerShouldSkipByCodexImportHealth(s.cfg, a) {
		GlobalModelRegistry().UnregisterClient(a.ID)
		return
	}
	if s != nil && s.cfg != nil {
		mergePersistedAuthRuntimeState(s.cfg.ConfigFilePath, a)
	}
	...
	authoritativeAPIKeyModels := false
	...
	case "gemini":
		...
		if entry := s.resolveConfigGeminiKey(a); entry != nil {
			if authKind == "apikey" {
				models = buildConfigModels(modeldiscovery.EffectiveGeminiKeyModels(s.cfg, serviceAuthConfigLocator(a), entry), "google", "gemini")
				authoritativeAPIKeyModels = true
	...
	case "vertex":
		...
		if entry := s.resolveConfigVertexCompatKey(a); entry != nil {
			if authKind == "apikey" {
				models = buildConfigModels(modeldiscovery.EffectiveVertexKeyModels(s.cfg, serviceAuthConfigLocator(a), entry), "google", "vertex")
				authoritativeAPIKeyModels = true
				excluded = entry.ExcludedModels
			} else if len(entry.Models) > 0 {
	...
	case "claude":
		...
		if entry := s.resolveConfigClaudeKey(a); entry != nil {
			if authKind == "apikey" {
				models = buildConfigModels(modeldiscovery.EffectiveClaudeKeyModels(s.cfg, serviceAuthConfigLocator(a), entry), "anthropic", "claude")
				authoritativeAPIKeyModels = true
				excluded = entry.ExcludedModels
			} else if len(entry.Models) > 0 {
	...
	case "codex":
		...
		if entry := s.resolveConfigCodexKey(a); entry != nil {
			if authKind == "apikey" {
				models = buildConfigModels(modeldiscovery.EffectiveCodexKeyModels(s.cfg, serviceAuthConfigLocator(a), entry), "openai", "openai")
				authoritativeAPIKeyModels = true
				excluded = entry.ExcludedModels
			} else if len(entry.Models) > 0 {
		...
		if s.cfg != nil {
			...
			for i := range s.cfg.OpenAICompatibility {
				...
				if strings.EqualFold(compat.Name, compatName) {
					isCompatAuth = true
					effectiveModels := modeldiscovery.EffectiveOpenAICompatModels(s.cfg, compat)
					// Convert compatibility models to registry models
					ms := make([]*ModelInfo, 0, len(effectiveModels))
					for j := range effectiveModels {
						m := effectiveModels[j]
					...
					if authKind == "apikey" {
						ms = applyAPIKeyLogicalModelGroups(s.cfg, ms)
					}
					// Register and return
					if len(ms) > 0 {
						if providerKey == "" {
	...
	if authKind == "apikey" {
		if !(authoritativeAPIKeyModels && len(models) == 0) {
			models = applyAPIKeyLogicalModelGroups(s.cfg, models)
		}
	}
	if len(models) > 0 {
		key := provider
...
func (s *Service) refreshAllModelRegistrations() int {
	if s == nil || s.coreManager == nil {
		return 0
	}
...
func (s *Service) refreshOpenAICompatDiscovery(ctx context.Context, previousCfg, cfg *config.Config) {
	_ = previousCfg
	if s == nil || cfg == nil {
		return
	}
	...
	timeoutCtx, cancel := context.WithTimeout(discoveryCtx, 30*time.Second)
	defer cancel()
	statuses, err := modeldiscovery.RescanConfigProviders(timeoutCtx, cfg, nil)
	if err != nil {
		log.Warnf("provider discovery refresh failed: %v", err)
		return
	}
	log.Infof("provider discovery refreshed for %d provider(s)", len(statuses))
}

func pendingOpenAICompatDiscoveryNames(previousCfg, cfg *config.Config) []string {
	if cfg == nil || len(cfg.OpenAICompatibility) == 0 {
		return nil
	}
	if previousCfg == nil {
		return openAICompatProvidersWithoutDeclaredModels(cfg)
	}

	previousByName := make(map[string]config.OpenAICompatibility, len(previousCfg.OpenAICompatibility))
	for i := range previousCfg.OpenAICompatibility {
		entry := previousCfg.OpenAICompatibility[i]
		...
		previousByName[name] = entry
	}

	out := make([]string, 0, len(cfg.OpenAICompatibility))
	seen := make(map[string]struct{}, len(cfg.OpenAICompatibility))
	for i := range cfg.OpenAICompatibility {
		...
		previous, ok := previousByName[key]
		if !ok || len(previous.Models) > 0 || !sameOpenAICompatDiscoveryInputs(previous, entry) {
			out = append(out, name)
		}
	}
	return out
...
func openAICompatProvidersWithoutDeclaredModels(cfg *config.Config) []string {
	if cfg == nil || len(cfg.OpenAICompatibility) == 0 {
		return nil
	}
	out := make([]string, 0, len(cfg.OpenAICompatibility))
...
func sameOpenAICompatDiscoveryInputs(a, b config.OpenAICompatibility) bool {
	if !strings.EqualFold(strings.TrimSpace(a.BaseURL), strings.TrimSpace(b.BaseURL)) {
		return false
	}
	if !equalOpenAICompatHeaders(a.Headers, b.Headers) {
		return false
	}
	return equalOpenAICompatAPIKeyEntries(a.APIKeyEntries, b.APIKeyEntries)
}

func equalOpenAICompatHeaders(a, b map[string]string) bool {
	if len(a) == 0 && len(b) == 0 {
		return true
	}
	normalizedA := make(map[string]string, len(a))
	for key, value := range a {
		...
		normalizedA[key] = value
	}
	normalizedB := make(map[string]string, len(b))
...
func equalOpenAICompatAPIKeyEntries(a, b []config.OpenAICompatibilityAPIKey) bool {
	normalize := func(items []config.OpenAICompatibilityAPIKey) []string {
		if len(items) == 0 {
			return nil
		}
		out := make([]string, 0, len(items))
		for i := range items {
			apiKey := strings.TrimSpace(items[i].APIKey)
			proxyURL := strings.TrimSpace(items[i].ProxyURL)
			if apiKey == "" && proxyURL == "" {
				continue
			}
			out = append(out, apiKey+"|"+proxyURL)
		}
		sort.Strings(out)
		return out
	}
...
func serviceAuthConfigLocator(auth *coreauth.Auth) string {
	if auth == nil || auth.Attributes == nil {
		return ""
	}
	return strings.TrimSpace(auth.Attributes["config_locator"])
}

func resolveServiceConfigEntryByLocator[T any](entries []T, auth *coreauth.Auth, prefix string) *T {
	locator := serviceAuthConfigLocator(auth)
	if locator == "" || prefix == "" {
		return nil
	}
	expectedPrefix := prefix + "["
	if !strings.HasPrefix(locator, expectedPrefix) || !strings.HasSuffix(locator, "]") {
		return nil
	}
	indexText := strings.TrimSuffix(strings.TrimPrefix(locator, expectedPrefix), "]")
...
func (s *Service) resolveConfigClaudeKey(auth *coreauth.Auth) *config.ClaudeKey {
	...
	if entry := resolveServiceConfigEntryByLocator(s.cfg.ClaudeKey, auth, "claude-api-key"); entry != nil {
		return entry
	}
...
func (s *Service) resolveConfigGeminiKey(auth *coreauth.Auth) *config.GeminiKey {
	...
	if entry := resolveServiceConfigEntryByLocator(s.cfg.GeminiKey, auth, "gemini-api-key"); entry != nil {
		return entry
	}
	var attrKey, attrBase string
	if auth.Attributes != nil {
...
func (s *Service) resolveConfigVertexCompatKey(auth *coreauth.Auth) *config.VertexCompatKey {
	...
	if entry := resolveServiceConfigEntryByLocator(s.cfg.VertexCompatAPIKey, auth, "vertex-api-key"); entry != nil {
		return entry
	}
	var attrKey, attrBase string
	if auth.Attributes != nil {
...
func (s *Service) resolveConfigCodexKey(auth *coreauth.Auth) *config.CodexKey {
	...
	if entry := resolveServiceConfigEntryByLocator(s.cfg.CodexKey, auth, "codex-api-key"); entry != nil {
		return entry
	}
	var attrKey, attrBase string
	if auth.Attributes != nil {
...
type resolvedNamedModelEntry struct {
	name string
}

func (e resolvedNamedModelEntry) GetName() string  { return e.name }
func (e resolvedNamedModelEntry) GetAlias() string { return e.name }

func buildResolvedNamedModels(models []string, ownedBy, modelType string) []*ModelInfo {
	if len(models) == 0 {
		return nil
	}
	entries := make([]resolvedNamedModelEntry, 0, len(models))
	for _, model := range models {
		if trimmed := strings.TrimSpace(model); trimmed != "" {
			entries = append(entries, resolvedNamedModelEntry{name: trimmed})
		}
	}
	return buildConfigModels(entries, ownedBy, modelType)
}
...
func applyOAuthModelAlias(cfg *config.Config, provider, authKind string, models []*ModelInfo) []*ModelInfo {
	...
	aliases := cfg.ProjectedOAuthModelAliases(channel)
	if len(aliases) == 0 {
		return models
	}
	return applyProjectedModelAliases(models, aliases)
}

func applyAPIKeyLogicalModelGroups(cfg *config.Config, models []*ModelInfo) []*ModelInfo {
	if cfg == nil {
		return models
	}

	groups := cfg.LogicalModelGroupEntries()
	if len(groups) == 0 {
		return models
	}

	aliases := make([]config.OAuthModelAlias, 0, len(groups))
	for i := range groups {
		aliases = append(aliases, config.OAuthModelAlias{
			Name:  groups[i].ResolvedTarget(),
			Alias: groups[i].Alias,
			Fork:  true,
		})
	}
	if len(models) == 0 {
		return buildLogicalModelGroupModels(groups)
	}
	return applyProjectedModelAliases(models, aliases)
}

type logicalModelAliasEntry struct {
	name  string
	alias string
}

func (e logicalModelAliasEntry) GetName() string  { return e.name }
func (e logicalModelAliasEntry) GetAlias() string { return e.alias }

func buildLogicalModelGroupModels(groups []config.LogicalModelGroup) []*ModelInfo {
	if len(groups) == 0 {
		return nil
	}
	entries := make([]logicalModelAliasEntry, 0, len(groups))
	for i := range groups {
		alias := strings.TrimSpace(groups[i].Alias)
		target := strings.TrimSpace(groups[i].ResolvedTarget())
		if alias == "" || target == "" {
			continue
		}
		entries = append(entries, logicalModelAliasEntry{name: target, alias: alias})
	}
	return buildConfigModels(entries, "logical-model-group", "logical-model-group")
}

func providerShouldSkipByCodexImportHealth(cfg *config.Config, auth *coreauth.Auth) bool {
	if cfg == nil || auth == nil {
		return false
	}
	...
	snapshot, ok := modeldiscovery.FindCodexImportHealthSnapshotByLocator(cfg.ConfigFilePath, locator)
	if !ok {
		return false
	}
	return modeldiscovery.IsCodexImportHardFailure(snapshot.Health.Status)
}

func (s *Service) resolveCodexImportSnapshotModels(auth *coreauth.Auth) ([]string, bool) {
	if s == nil || s.cfg == nil || auth == nil || auth.Attributes == nil {
		return nil, false
	}
	locator := strings.TrimSpace(auth.Attributes["config_locator"])
	if locator == "" {
	...
	return modeldiscovery.CodexImportEffectiveModelsByLocator(s.cfg.ConfigFilePath, locator)
}

func applyProjectedModelAliases(models []*ModelInfo, aliases []config.OAuthModelAlias) []*ModelInfo {
