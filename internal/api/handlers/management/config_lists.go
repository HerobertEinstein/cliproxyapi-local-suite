...
func (h *Handler) PutGeminiKeys(c *gin.Context) {
	oldEntries := append([]config.GeminiKey(nil), h.cfg.GeminiKey...)
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	h.persistWithConfigProviderRefresh(c, geminiConfigProviderRefreshKeys(oldEntries, h.cfg.GeminiKey))
}
func (h *Handler) PatchGeminiKey(c *gin.Context) {
	...
	if body.Value.APIKey != nil {
		...
		if trimmed == "" {
			...
			h.cfg.SanitizeGeminiKeys()
			h.persistWithConfigProviderRefreshPlan(c, geminiConfigProviderRefreshPlan(oldEntries, h.cfg.GeminiKey))
			return
		}
...
func (h *Handler) PutClaudeKeys(c *gin.Context) {
	oldEntries := append([]config.ClaudeKey(nil), h.cfg.ClaudeKey...)
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	h.persistWithConfigProviderRefresh(c, claudeConfigProviderRefreshKeys(oldEntries, h.cfg.ClaudeKey))
}
func (h *Handler) PatchClaudeKey(c *gin.Context) {
	...
	h.cfg.SanitizeClaudeKeys()
	h.persistWithConfigProviderRefreshPlan(c, claudeConfigProviderRefreshPlan(oldEntries, h.cfg.ClaudeKey))
}
...
func (h *Handler) PutOpenAICompat(c *gin.Context) {
	oldEntries := append([]config.OpenAICompatibility(nil), h.cfg.OpenAICompatibility...)
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	h.persistWithConfigProviderRefresh(c, openAICompatConfigProviderRefreshKeys(oldEntries, h.cfg.OpenAICompatibility))
}
func (h *Handler) PatchOpenAICompat(c *gin.Context) {
	...
	if body.Value.BaseURL != nil {
		...
		if trimmed == "" {
			...
			h.cfg.SanitizeOpenAICompatibility()
			h.persistWithConfigProviderRefreshPlan(c, openAICompatConfigProviderRefreshPlan(oldEntries, h.cfg.OpenAICompatibility))
			return
		}
...
func (h *Handler) PutVertexCompatKeys(c *gin.Context) {
	oldEntries := append([]config.VertexCompatKey(nil), h.cfg.VertexCompatAPIKey...)
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	h.persistWithConfigProviderRefresh(c, vertexConfigProviderRefreshKeys(oldEntries, h.cfg.VertexCompatAPIKey))
}
func (h *Handler) PatchVertexCompatKey(c *gin.Context) {
	...
	if body.Value.APIKey != nil {
		...
		if trimmed == "" {
			...
			h.cfg.SanitizeVertexCompatKeys()
			h.persistWithConfigProviderRefreshPlan(c, vertexConfigProviderRefreshPlan(oldEntries, h.cfg.VertexCompatAPIKey))
			return
		}
		entry.APIKey = trimmed
	}
...
func (h *Handler) PutCodexKeys(c *gin.Context) {
	oldEntries := append([]config.CodexKey(nil), h.cfg.CodexKey...)
	data, err := c.GetRawData()
	if err != nil {
		c.JSON(400, gin.H{"error": "failed to read body"})
		return
	...
	h.persistWithConfigProviderRefresh(c, codexConfigProviderRefreshKeys(oldEntries, h.cfg.CodexKey))
}
func (h *Handler) PatchCodexKey(c *gin.Context) {
	...
	if body.Value.BaseURL != nil {
		...
		if trimmed == "" {
			...
			h.cfg.SanitizeCodexKeys()
			h.persistWithConfigProviderRefreshPlan(c, codexConfigProviderRefreshPlan(oldEntries, h.cfg.CodexKey))
			return
		}
		entry.BaseURL = trimmed
	}
