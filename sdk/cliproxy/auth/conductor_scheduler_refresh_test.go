...
func TestManager_SchedulerLogicalAliasCooldownUsesResolvedUpstreamModel(t *testing.T) {
	ctx := context.Background()
	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	...
	cfg := &internalconfig.Config{
		...
		LogicalModelGroups: internalconfig.LogicalModelGroups{
			Current: internalconfig.LogicalModelCurrent{Ref: "gpt-5.2"},
	...
	registerSchedulerModels(t, "codex", "current", auth.ID)

	if _, errRegister := manager.Register(ctx, auth); errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	}

	got, errPick := manager.scheduler.pickSingle(ctx, "codex", "current", cliproxyexecutor.Options{}, nil)
	var cooldownErr *modelCooldownError
	if !errors.As(errPick, &cooldownErr) {
