...
## Windows GUI launcher and Edge CDP

If you open the bundled Web UI on Windows through a shortcut, tray entry, or helper script and also want browser automation tools to attach to the same real Edge session, keep the launch chain consistent:

- launch Microsoft Edge with `--remote-debugging-port=9333`
- do not bypass that flag in secondary helpers such as `--app=...` wrappers
- if you maintain your own launcher, pass the same flag in both app-mode and normal window-mode launches

This matters because Playwright / CDP attachment depends on the actual Edge process command line, not just on the shortcut you expected to be used. If one helper starts `msedge.exe --app=...` without the remote debugging port, the Web UI still opens, but automation attaches will fail against the existing Edge window.
